...
double ds(const double r, void *p) {
  std::vector<double> * params = reinterpret_cast<std::vector<double> *>(p);
  double a = (*params)[0];
  double delta;
  delta = r * r - 2. * r + a * a;
...
double calds_eq(const double a, const double r1, const double r2) {
    ...
    double inte = 0.;
    gauleg(r1, r2, abissca, weights, NGAUSS);
    
    std::vector<double> params = {a};
...
/*!
Calculates the specific intensity of the blackbody radiation
...
/*!
Calculates the specific intensity (in ) of the blackbody radiation at a particular frequency nu
@param tbb photon temperature in kev
@param nu frequency in [Hz]
*/
double planck(const double tbb, const double nu) {
    double ratio = H * nu / tbb;
    double norm = 2. * H_ERG * nu * nu * nu / (LSPEED * LSPEED);
    if (ratio >= 100.) {
        return norm * std::exp(-1. * ratio);
    } else if (ratio <= 1e-4) {
        return norm / ratio;
    } else { 
        return norm / (std::exp(ratio) - 1.);
    }
}
...
/*! \brief Return orthonormal tetrad given fluid four-velocity, following Schnittman & Krolik 2013, Eq. 12

@param umu The four velocity of the fluid
@param met The metric; a `sim5metric` object
@param tet The tetrad; a `sim5tetrad` object
*/
void caltetrad(const std::array<double, 4> & umu, sim5metric * met, sim5metric * met_up, sim5tetrad * tet) {
    double u_t, u_phi;
    double C0, C0sqr, C1, C1sqr, C2, C3, one_over_N1, one_over_N2, one_over_N3;
    
    u_t = met->g00 * umu[0] + met->g03 * umu[3];
    u_phi = met->g33 * umu[3] + met->g03 * umu[0];
    
    C0 = u_phi / u_t;
    C0sqr = C0 * C0;
    C1 = umu[1] / (umu[0] + C0 * umu[3]);
    C1sqr = C1 * C1;
    C2 = (met_up->g00 + 2. * C0 * met_up->g03 + C0sqr * met_up->g33) / met_up-> g11;
    C3 = umu[0] + C1 * C2 * umu[1] + C0 * umu[3];
    
    one_over_N1 = 1. / std::sqrt(met->g00 * C0sqr - 2. * met->g03 * C0 + met->g33);
    one_over_N2 = 1. / std::sqrt(C1sqr * (met_up->g00 + 2. * met_up->g03 * C0 + met_up->g33 * C0sqr) + met_up->g11);
    one_over_N3 = 1. / std::sqrt(umu[2] * umu[2] * (met_up->g00 + 2. * met->g03 * C0 + met_up->g11 * C1sqr * C2 * C2
        + met_up->g33 * C0sqr) + met_up->g22 * C3 * C3);
    ...
    tet->e[1][0] = -1. * one_over_N2 * (met_up->g00 + met_up->g03 * C0) * C1;
    tet->e[1][1] = one_over_N2 * met_up->g11;
    tet->e[1][2] = 0.;
    tet->e[1][3] = -1. * one_over_N2 * (met_up->g03 + met_up->g33 * C0) * C1;
    
    tet->e[2][0] = -1. * one_over_N1 * C0;
    tet->e[2][1] = 0.;
    tet->e[2][2] = 0.;
    tet->e[2][3] = one_over_N1;
    
    tet->e[3][0] = one_over_N3 * (met_up->g00 + met_up->g03 * C0) * umu[2];
    tet->e[3][1] = one_over_N3 * met_up->g11 * C1 * C2 * umu[2];
    tet->e[3][2] = -1. * one_over_N3 * met_up->g22 * C3;
    tet->e[3][3] = one_over_N3 * (met_up->g03 + met_up->g33 * C0) * umu[2];
