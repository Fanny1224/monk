...
bool geoinf::ispossible(const tridgeo & trid, const double r, const double mu, const double kr, const double kmu) {
    bool status = true;
    if (q >= 0.) {
        if ((trid.mumin > muplus) || (trid.mumax < muminus) || (nrr == 2 && trid.rmax < rr1) ||
        ...
        //if (trid.mumin > 0. && trid.mumax > 0. && kmu >= 0. && mu < trid.mumin) {
        //    status = false;
        //}
        if (kr <= 0. && r < trid.rmin) {
        ...
        if ((mu > 0. && (trid.mumin > muplus || trid.mumax < muminus)) || (mu < 0. && (trid.mumin < -1. * muplus || trid.mumax > -1. * muminus)))
            status = false;
        if (kr >= 0. && r > trid.rmax)
            status = false;
