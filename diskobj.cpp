...
void nt::angles(const double r, const double l, const double q, const double signkr, std::array<double, 4> & cosang) const {
...
//! @param l see `angles()`.
//! @param q see `angles()`.
std::complex<double> nt::calwp(const double r, const double l, const double q) const {
...
//! @param l \f$\equiv L_z/E_\infty\f$, where \f$L_z\f$ is the photon angular momentum about the symmetry axis,
//!     and \f$E_\infty\f$ is the photon energy at infinity
std::complex<double> nt::calwp_angle(const double r, const double theta, const double phi, const double l) const {
    double vphival, Gamma, Delta, A0, X, Y, Eloc, Knorm, K1, K2;
    double cost, sint, cosp, sinp;
...
*/
std::complex<double> nt::calwp_angle(const double r, const double l, const double polang, const std::array<double, 4> & angles) const {
    ...
    c1 = angles[0] * angles[2] * X;
    c2 = (angles[3] * X + angles[1] * Y);
...
std::complex<double> nt::calwp_angle(const double r, const double theta, const double phi, const double l, const double polang) const {
    std::array<double, 4> angles = {std::cos(theta), std::sin(theta), std::cos(phi), std::sin(phi)};
    return calwp_angle(r, l, polang, angles);
}
...
*/
void nt::polrotang_disktok(const double r, const double l, const double q, const std::array<double, 4> angles, double & c1, double & c2)
    const {
    ...
    c1 = Eloc * Gamma * r * angles[0] * angles[2] * X * cnorm;
    c2 = Eloc * Gamma * r * (angles[3] * X + angles[1] * Y) * cnorm;
}
...
void nt::polrotang_disktok(const double r, const double theta, const double phi, const double l, const double q, double & c1, double & c2) 
    const {
    std::array<double, 4> angles = {std::cos(theta), std::sin(theta), std::cos(phi), std::sin(phi)};
    polrotang_disktok(r, l, q, angles, c1, c2);
}
...
*/
double nt::polrotang_disktok(const double r, const double theta, const double phi, const double l, const double q) const {
    double c1, c2, ang;
...
double nt::polrotang_disktok(const double r, const double l, const double q, const std::array<double, 4> & angles) const {
    double c1, c2, ang;
    polrotang_disktok(r, l, q, angles, c1, c2);
...
void nt::mkrgrid(const long nr, const double rin, const double rout, std::vector<double> & radius) const {
    double rinnow = (rin < rms) ? rms : rin;
    mkloggrid(nr, rinnow, rout, radius);
}
