...
void singledet_sp::calflux(const std::vector<double> & enarr, const std::vector<double> & warr, const std::vector<bool> & cond) {
    ...
    if (linear) {
        ...
        double one_over_dloge = 1. / std::log10(en[1] / en[0]);
        double logemin = std::log10(emin);
        
        for (size_t it = 0; it < enarr.size(); ++it) {
            index = std::floor((std::log10(enarr[it]) - logemin) * one_over_dloge);
            if (cond[it] && index >= 0 && (size_t)(index) < flux.size()) {
                flux[index] += warr[it];
            }
        }
...
void singledet_sp::callc(const double phimin, const double phimax, const std::vector<double> & enarr, const std::vector<double> & weight,
    const std::vector<double> & muinf, const std::vector<double> & phiinf, const std::vector<double> & tinf) {
    
    std::vector<bool> cond(enarr.size());
    for (size_t it = 0; it < enarr.size(); ++it) {
        cond[it] = detected(muinf[it]) && phiinf[it] >= phimin && phiinf[it] < phimax;
    }
    calflux(tinf, weight, cond);
}

void singledet_sp::callc(const double phimin, const double phimax, const double emin, const double emax, 
    const std::vector<double> & enarr, const std::vector<double> & weight,
    const std::vector<double> & muinf, const std::vector<double> & phiinf, const std::vector<double> & tinf) {
    
    std::vector<bool> cond(enarr.size());
    for (size_t it = 0; it < enarr.size(); ++it) {
        cond[it] = detected(muinf[it]) && phiinf[it] >= phimin && phiinf[it] < phimax && enarr[it] >= emin && enarr[it] <= emax;
    }
    calflux(tinf, weight, cond);
}
